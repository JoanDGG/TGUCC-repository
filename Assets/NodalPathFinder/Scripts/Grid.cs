...
public class Grid : MonoBehaviour {
	...
	{	
        ...
        {
            NPCPath.AddRange(DrawPath(gameObjects[i], positions[i].position));
        }

        foreach (Vector2 position in NPCPath)
        {
            print(position);
        }
        // -----------------
    ...
    public List<Vector2> DrawPath(GameObject gameObject, Vector2 destination)
    {
        //Convert GridPoints to grid coordinates
        Point gridPos = WorldToGrid(destination);
        ...
        if (gridPos != null)
        {
            ...
            if (gridPos.X > 0 && gridPos.Y > 0 && gridPos.X < Width && gridPos.Y < Height)
            {
                ...
                Point playerPos = WorldToGrid(gameObject.transform.position);
                ...
                while (bc != null)
                {
                    Vector2 position = Grid.GridToWorld(bc.position);
                    lr.SetPosition(count, position);
